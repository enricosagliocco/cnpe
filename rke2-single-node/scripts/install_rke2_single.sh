...
RKE2_SINGLE_NAME="${RKE2_SINGLE_NAME:-rke2-single}"

echo "==> Installo RKE2 server (single node control-plane + worker)"
...
echo "==> Forzo il ruolo worker sul nodo single-node"
NODE_NAME="$(hostname -s)"
sudo /var/lib/rancher/rke2/bin/kubectl label node "${NODE_NAME}" node-role.kubernetes.io/worker=true --overwrite || true

# In single-node vogliamo workload applicativi anche sul nodo control-plane.
sudo /var/lib/rancher/rke2/bin/kubectl taint nodes "${NODE_NAME}" node-role.kubernetes.io/control-plane- || true
sudo /var/lib/rancher/rke2/bin/kubectl taint nodes "${NODE_NAME}" node-role.kubernetes.io/master- || true
sudo /var/lib/rancher/rke2/bin/kubectl taint nodes "${NODE_NAME}" node-role.kubernetes.io/etcd- || true
...
echo
echo "Nodo single-node configurato."
echo "Comandi utili:"
