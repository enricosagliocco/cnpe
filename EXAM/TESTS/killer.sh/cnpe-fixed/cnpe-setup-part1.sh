...
ensure_gitea_org() {
  ...
  [ "$code" = "201" ] || [ "$code" = "200" ] || [ "$code" = "409" ] || warn "Org ${GITEA_ORG} may already exist or cannot be created (HTTP ${code})"
}
gitea_create_repo() {
  local repo=$1 code
  ...
  [ "$code" = "201" ] || [ "$code" = "200" ] || [ "$code" = "409" ] || die "Cannot create/access repo ${GITEA_ORG}/${repo} (HTTP ${code})"
}
