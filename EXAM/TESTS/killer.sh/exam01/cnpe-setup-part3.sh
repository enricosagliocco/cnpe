...
kubectl create ns observability --dry-run=client -o yaml | kubectl apply -f - >/dev/null
...
curl -sSL https://github.com/argoproj/argo-rollouts/releases/latest/download/kubectl-argo-rollouts-linux-amd64 \
  -o /usr/local/bin/kubectl-argo-rollouts 2>/dev/null && \
  chmod +x /usr/local/bin/kubectl-argo-rollouts || true
