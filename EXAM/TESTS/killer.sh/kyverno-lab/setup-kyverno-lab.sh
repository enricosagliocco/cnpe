...
metadata: {name: require-ns-annotation}
spec:
...
apiVersion: v1
kind: Namespace
metadata: {name: kyverno-project-bad}
YAML
...
metadata:
  ...
  annotations: {project-name: payments}
YAML

cat > "$COURSE_DIR/02/policy.yaml" <<'YAML'
apiVersion: policies.kyverno.io/v1
kind: NamespacedMutatingPolicy
metadata: {name: mutate-pods, namespace: apps}
spec:
  matchConstraints:
    resourceRules:
      - apiGroups: [""]
...
apiVersion: v1
kind: Pod
metadata: {name: metadata-demo, namespace: apps}
...
apiVersion: policies.kyverno.io/v1
kind: ValidatingPolicy
metadata: {name: require-deployment-labels}
spec:
...
apiVersion: apps/v1
kind: Deployment
metadata: {name: labels-bad, namespace: apps}
spec:
...
metadata:
  name: labels-good
  namespace: apps
  labels: {app.kubernetes.io/name: labels-good, owner: platform}
spec:
...
apiVersion: policies.kyverno.io/v1
kind: ValidatingPolicy
metadata: {name: minimum-replicas}
spec:
  validationActions: [Deny]
  matchConstraints:
    resourceRules:
...
apiVersion: apps/v1
kind: Deployment
metadata: {name: replicas-${mode}, namespace: production, labels: {environment: production}}
spec:
  replicas: ${replicas}
  selector: {matchLabels: {app: replicas-${mode}}}
  template:
    metadata: {labels: {app: replicas-${mode}}}
    spec: {containers: [{name: app, image: nginx:1-alpine}]}
YAML
done
...
apiVersion: v1
kind: Pod
metadata: {name: resources-bad, namespace: apps}
spec:
  initContainers: [{name: init, image: busybox:1.36, command: [sh, -c, "true"]}]
  containers: [{name: app, image: nginx:1-alpine}]
YAML
cat > "$COURSE_DIR/05/good.yaml" <<'YAML'
apiVersion: v1
kind: Pod
metadata: {name: resources-good, namespace: apps}
spec:
  containers:
    - name: app
...
metadata: {name: allowed-registries, namespace: apps}
spec:
  validationActions: [Deny]
  matchConstraints:
    resourceRules:
...
apiVersion: v1
kind: Pod
metadata: {name: registry-bad, namespace: apps}
spec:
...
apiVersion: v1
kind: Pod
metadata: {name: registry-good, namespace: apps}
spec: {containers: [{name: app, image: ghcr.io/company/api:1.0.0}]}
YAML
...
apiVersion: v1
kind: Pod
metadata: {name: image-${name}, namespace: apps}
...
apiVersion: v1
kind: Pod
metadata: {name: pss-bad, namespace: apps}
spec: {containers: [{name: app, image: nginx:1-alpine}]}
YAML
cat > "$COURSE_DIR/08/good.yaml" <<'YAML'
apiVersion: v1
kind: Pod
metadata: {name: pss-good, namespace: apps}
spec:
...
apiVersion: policies.kyverno.io/v1
kind: ValidatingPolicy
metadata: {name: secure-containers}
spec:
  validationActions: [Deny]
  matchConstraints:
    resourceRules:
...
apiVersion: v1
kind: Pod
metadata: {name: privilege-bad, namespace: apps}
spec:
  containers:
    - name: app
      image: nginx:1-alpine
...
apiVersion: v1
kind: Pod
metadata: {name: privilege-good, namespace: apps}
spec:
  containers:
    - name: app
      image: nginx:1-alpine
...
apiVersion: policies.kyverno.io/v1
kind: ValidatingPolicy
metadata: {name: disallow-hostpath}
spec:
  validationActions: [Deny]
  matchConstraints:
    resourceRules:
...
apiVersion: v1
kind: Pod
metadata: {name: hostpath-bad, namespace: apps}
spec:
  volumes: [{name: host, hostPath: {path: /var/lib}}]
  containers: [{name: app, image: busybox:1.36, command: [sh, -c, "sleep 3600"], volumeMounts: [{name: host, mountPath: /host}]}]
YAML
cat > "$COURSE_DIR/10/good.yaml" <<'YAML'
apiVersion: v1
kind: Pod
metadata: {name: hostpath-good, namespace: apps}
spec:
  volumes: [{name: data, emptyDir: {}}]
  containers: [{name: app, image: busybox:1.36, command: [sh, -c, "sleep 3600"], volumeMounts: [{name: data, mountPath: /data}]}]
YAML

cat > "$COURSE_DIR/11/policy.yaml" <<'YAML'
apiVersion: policies.kyverno.io/v1
kind: NamespacedValidatingPolicy
metadata: {name: restrict-service-types, namespace: production}
spec:
  validationActions: [Deny]
  matchConstraints:
    resourceRules:
...
apiVersion: v1
kind: Service
metadata: {name: service-${type}, namespace: production}
spec:
  ${extra}
  selector: {app: demo}
...
apiVersion: policies.kyverno.io/v1
kind: ValidatingPolicy
metadata: {name: require-ingress-tls}
spec:
  validationActions: [Deny]
  matchConstraints:
    resourceRules:
...
kind: Ingress
metadata: {name: ingress-bad, namespace: apps}
spec:
  rules: [{host: api.example.com, http: {paths: [{path: /, pathType: Prefix, backend: {service: {name: api, port: {number: 80}}}}]}}]
YAML
cat > "$COURSE_DIR/12/good.yaml" <<'YAML'
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata: {name: ingress-good, namespace: apps}
spec:
...
apiVersion: apps/v1
kind: Deployment
metadata: {name: immutable-demo, namespace: apps, labels: {team: platform, version: v1}}
spec:
  selector: {matchLabels: {app: immutable-demo}}
  template:
    metadata: {labels: {app: immutable-demo}}
    spec: {containers: [{name: app, image: nginx:1-alpine}]}
YAML
...
apiVersion: v1
kind: Pod
metadata: {name: selector-${ns}, namespace: ${ns}}
spec: {containers: [{name: app, image: registry.k8s.io/pause:3.10}]}
YAML
done
...
apiVersion: policies.kyverno.io/v1
kind: NamespacedValidatingPolicy
metadata: {name: team-a-owner, namespace: team-a}
spec:
  validationActions: [Deny]
  matchConstraints:
    resourceRules:
...
apiVersion: apps/v1
kind: Deployment
metadata: {name: audit-demo, namespace: apps}
spec:
  selector: {matchLabels: {app: audit-demo}}
  template:
    metadata: {labels: {app: audit-demo}}
    spec: {containers: [{name: app, image: nginx:1-alpine}]}
YAML
...
apiVersion: policies.kyverno.io/v1
kind: ValidatingPolicy
metadata: {name: required-owner-message}
spec:
  validationActions: [Deny]
  matchConstraints:
    resourceRules:
...
apiVersion: policies.kyverno.io/v1
kind: NamespacedMutatingPolicy
metadata: {name: default-environment, namespace: apps}
spec:
  matchConstraints:
    resourceRules:
      - apiGroups: [""]
...
apiVersion: policies.kyverno.io/v1
kind: NamespacedValidatingPolicy
metadata: {name: final-validation, namespace: apps}
spec:
  validationActions: [Audit] # TODO Deny
  matchConstraints:
    resourceRules:
      - apiGroups: [""]
        apiVersions: ["v1"]
        operations: ["CREATE"]
        resources: ["configmaps"] # TODO pods
  validations:
...
apiVersion: policies.kyverno.io/v1
kind: NamespacedMutatingPolicy
metadata: {name: final-mutation, namespace: apps}
spec:
  matchConstraints:
    resourceRules:
      - apiGroups: [""]
...
apiVersion: v1
kind: Pod
metadata: {name: final-bad, namespace: apps}
spec: {containers: [{name: app, image: registry.k8s.io/pause:3.10}]}
YAML
