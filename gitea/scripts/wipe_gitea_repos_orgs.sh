...
Opzioni:
  ...
  --url <URL>       Base URL Gitea (default: env GITEA_URL o http://158.180.234.164:3000)
...
delete_org() {
  ...
  if [[ "$DRY_RUN" == true ]]; then
    echo "[DRY-RUN] delete org ${org}"
    return 0
  fi

  local code
  code="$(api_status DELETE "/api/v1/orgs/${org}")"
  if [[ "$code" == "204" || "$code" == "200" ]]; then
    echo "[OK] deleted org ${org}"
  else
    warn "delete org ${org} failed (HTTP ${code})"
  fi
}
      ...
      -h|--help)
        ...
        require_confirmation_guard() {
          if [[ "$DRY_RUN" == false && "${GITEA_WIPE_FORCE:-false}" != true ]]; then
            case "${GITEA_URL%/}" in
              http://158.180.234.164:3000|https://158.180.234.164:3000|http://gitea.local:3000|https://gitea.local:3000)
                ;;
              *)
